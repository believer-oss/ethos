...
use ethos_core::types::repo::CheckoutCommitRequest;
...
impl Task for CheckoutOp {
    async fn execute(&self) -> Result<(), CoreError> {
        let checkout_arg = self.commit.as_ref().unwrap_or(&self.branch);
        self.git_client
            .run(&["checkout", checkout_arg], Default::default())
            .await
...
pub async fn checkout_commit_handler<T>(
    State(state): State<AppState<T>>,
    Json(request): Json<CheckoutCommitRequest>,
) -> Result<Json<String>, CoreError>
where
    T: EngineProvider,
{
    ...
    let op = {
        ...
        CheckoutOp {
            repo_path: app_config.repo_path.clone(),
            branch: repo_config.trunk_branch.clone(),
            commit: Some(request.commit.clone()),
            git_client: state.git(),
        }
    };
