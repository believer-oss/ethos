...
<div class="flex items-center justify-between gap-2">
	<div class="flex items-center gap-2">
		<p class="text-2xl my-2 dark:text-primary-400">Logs</p>
		<Button disabled={loading} class="!p-1.5" primary on:click={refreshLogs}>
			<RotateOutline class="w-4 h-4" />
		</Button>
		<Label class="text-sm">Filter by level:</Label>
		<MultiSelect
...
<Card
	class="w-full p-4 sm:p-4 max-w-full flex-grow overflow-y-auto dark:bg-secondary-600 border-0 shadow-none"
>
	<div>
		<TableSearch
		...
		>
			<TableBody>
				{#each filteredItems as log}
					<TableBodyRow class="text-left border-b-0 p-2 bg-secondary-600">
						<TableBodyCell class="p-2 py-1 w-52">
							<p class="text-sm dark:text-gray-300">{log.timestamp}</p>
						</TableBodyCell>
						<TableBodyCell class="text-left border-b-0 p-2 py-1 w-20">
							<p class="text-sm dark:text-primary-400">
								<Badge class="dark:text-white w-full h-full {getBadgeClass(log.level)}"
									>{log.level}</Badge
								>
							</p>
						</TableBodyCell>
						<TableBodyCell class="text-left border-b-0 p-2 py-1">
							<p class="text-sm dark:text-primary-400 whitespace-normal">
